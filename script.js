...
// function to transform the metadate encoded in teiHeader with the xsl stylesheet "Frankenstein_meta.xsl", this will apply the templates and output the text in the html <div id="stats">
  ...
  var PercyArray = Array.from(visible_percy);
    ...
    visible_mary.forEach(elem => { elem.style.color = 'black'; elem.style.display = 'inline'; });
    visible_percy.forEach(elem => { elem.style.color = 'black'; elem.style.display = 'inline'; });
  } 
  else if (event.target.value == 'Mary') {
    visible_mary.forEach(elem => { elem.style.color = 'red'; elem.style.display = 'inline'; });
    visible_percy.forEach(elem => { elem.style.color = 'black'; elem.style.display = 'inline'; });
  } 
  else if (event.target.value == 'Percy') {
    visible_percy.forEach(elem => { elem.style.color = 'blue'; elem.style.display = 'inline'; });
    visible_mary.forEach(elem => { elem.style.color = 'black'; elem.style.display = 'inline'; });
  }
 }
